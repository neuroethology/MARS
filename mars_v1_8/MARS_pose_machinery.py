...
def post_proc_heatmaps(predicted_heatmaps, bboxes, IM_W, IM_H, POSE_IM_SIZE):
    ...
    for b in range(len(predicted_heatmaps[0])):
        ...
        keypoints = np.zeros((NUM_PARTS, 6))
        ...
        for j in range(NUM_PARTS):
            ...
            s_y = np.sum(hm, axis = 1)  # (N, K, H)
            s_x = np.sum(hm, axis = 0)  # (N, K, W)
            
            y = np.linspace(-1.0, 1.0, H)
            x = np.linspace(-1.0, 1.0, W)
            
            u_y = np.sum(y * s_y, axis = 0) / np.sum(s_y, axis = 0)  # (N, K)
            u_x = np.sum(x * s_x, axis = 0) / np.sum(s_x, axis = 0)

            y = np.reshape(y, (H, 1))
            x = np.reshape(x, (1, W))
            
            # Covariance
            var_y = (np.sum(hm * y * y) - u_y * u_y) + 1e-6
            var_x = (np.sum(hm * x * x) - u_x * u_x) + 1e-6
            
            cov = np.sum(hm * (x - u_x) * (y - u_y))
                    
            # Extract out the keypoint.
            x, y = np.array(np.unravel_index(np.argmax(hm), hm.shape)[::-1])

            # print(x,y, (u_x+1)/2* W, (u_y+1)/2*H)
            ...
            # Store it.
            keypoints[j, :] = [imx, imy, score, var_x, var_y, cov]
        ...
        covs.append([keypoints[:, 3].tolist(), keypoints[:, 4].tolist(), keypoints[:, 5].tolist()])
    return keypoints_res, scores, covs
...
def post_hm_setup(NUM_FRAMES):
    ...
    pose_frames = {'scores': [], 'keypoints': [],'bbox':[],'bscores':[], 'covs': []}
...
def post_hm_inner(predicted_heatmaps, bboxes_confs, IM_W, IM_H, POSE_IM_SIZE, NUM_FRAMES, POSE_BASENAME, pose_frames, bar, current_frame_num):
    ...
    keypoints_res, scores, covs = post_proc_heatmaps(predicted_heatmaps, bboxes_confs[0], IM_W, IM_H, POSE_IM_SIZE)
